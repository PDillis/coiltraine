...
class RandomSampler(Sampler):
    ...
    Arguments:
        keys (list): keys to sample from
        executed_iterations (int): iterations executed so far
    """
...
class PreSplitSampler(Sampler):
    """
    Sample on a list of keys that was previously split
    Arguments:
        keys (list): keys to sample from
        executed_iterations (int): iterations executed so far
        weights (list): weights to sample keys from
    """
...
class LogitSplitSampler(Sampler):
    """
    Sample on a list of keys that was previously split;
...
class BatchSequenceSampler(object):
    ...
    def __init__(self, keys, executed_iterations,
        ...
        sampler = PreSplitSampler(keys, executed_iterations)
